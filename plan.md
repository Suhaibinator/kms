...
type SecretValue struct {
    ...
    BindKey string // operator-owned binding key required for bound versions
...
DEK
   ...
   | (derived via HKDF from the operator-owned binding key)
   v
inner-wrapped DEK
   |
...
On read and write, the client supplies its binding key with the request. The server unwraps the outer layer with the master key, derives the client key from the supplied binding key, unwraps the inner layer, decrypts the value, and discards the binding key and derived key from memory. The binding key is never persisted, hashed, or fingerprinted server-side.
...
3. There is deliberately no recovery escrow. Loss of either the master key or the operator-owned binding key makes the secret permanently unrecoverable. Opting a secret into client-bound mode is an explicit acceptance of this risk, and the frontend/CLI must state it at creation time.
...
2. Bound secret versions additionally require their operator-owned binding key for decryption.
...
3. Tokens must be revocable and rotatable per client.
...
For client-bound secrets there is deliberately no recovery escrow: loss of either the master key or the operator-owned binding key permanently destroys the secret. This is by design and must be documented, not mitigated.
...
1. Token-based machine authentication (per-client identity tokens).
...
Authentication:
  bearer access tokens for service clients (per-client identity tokens)
  mTLS as later production hardening
  bootstrap admin token or reverse-proxy auth for frontend initially

Client-bound secrets:
  opt-in per secret; DEK double-wrapped (binding-key-derived key, then KEK)
...
2. Emit source-to-destination mappings so application paths can be updated.

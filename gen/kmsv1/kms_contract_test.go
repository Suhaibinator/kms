...
func TestV03WireFieldLayouts(t *testing.T) {
	...
	tests := []struct {
		...
		{"ReleaseAcknowledgement", fields("namespace", "name", "version", "activation_revision", "client_name", "instance_id", "state", "rejection_category", "diagnostic", "timestamp_unix_ms", "applied_divergent", "divergent_field_count", "schema_version", "sequence", "session_id", "target_revision")},
		{"ReleaseAcknowledgementRejectedEvent", fields("namespace", "name", "schema_version", "version", "activation_revision", "client_name", "instance_id", "state", "sequence", "reason", "session_id", "target_revision")},

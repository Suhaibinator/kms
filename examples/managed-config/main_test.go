...
func TestRunDemonstratesManagedConfigurationLifecycle(t *testing.T) {
	...
	for _, milestone := range []string{
		"applied: phase=startup release=dev/managed-config-example/runtime@1#1 schema_version=1 divergent=false changed=(none)",
		"initial typed snapshot: release=1",
		"api_key=[REDACTED] api_key_version=1",
		"applied: phase=runtime release=dev/managed-config-example/runtime@2#2 schema_version=1 divergent=true changed=api_key,runtime.greeting,runtime.request_limit",
		...
		"applied: phase=runtime release=dev/managed-config-example/runtime@4#4 schema_version=1 divergent=false changed=api_key,runtime.greeting,runtime.request_limit",

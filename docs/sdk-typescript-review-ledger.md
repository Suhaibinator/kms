...
| Namespace scope, background reconciliation, and deleted-path history were append-only after local unsubscribe or unique-key churn; an already in-flight reconcile page could also repopulate a released scope, and delete-only unknown keys bypassed the original compaction path. | Added namespace reference ownership, idle stream/reconcile suspension, tombstone compaction, and a global stale-reconcile fence (`1b90753`); fenced each awaited page by captured scope generation and current ownership (`0a4bd60`); and pruned unchanged unknown tombstones without inventing callbacks (`a0c7943`). | `tests/stress.test.ts`, `tests/watch.test.ts` |

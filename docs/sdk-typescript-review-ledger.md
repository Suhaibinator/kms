...
| Namespace discovery inherited the first caller's cancellation and missed per-call deadlines. | Made discovery client-owned with caller-local cancellation and deadlines. | `tests/client.test.ts`, `tests/grpc-integration.test.ts` |
...
| Ordinary reads trusted a missing, wrong-resource, or wrong-version response, and local `AbortError` values could be mistaken for numeric gRPC status errors. | Validate every returned resource identity and concrete version before use (`e033de8`); preserve local cancellation while recognizing genuine gRPC errors across duplicated `grpc-js` installs (`cbf006e`, `d68aac4`). | `tests/client.test.ts`, `tests/errors.test.ts` |
| Synchronous abort listeners could re-enter `close()` before its shared result was installed. | Publish the client close attempt before aborting owned work and prove one transport cleanup/result (`ae535de`). | `sdk/typescript/tests/client.test.ts` |
...
| A read started before a mutation or watch invalidation could complete afterward and repopulate stale parameter or secret data for the full TTL. | Added bounded, per-kind/path active-read generations; point, namespace, watch, mutation, clear, and close invalidations now fence late cache fills (`e033de8`). | `tests/cache.test.ts`, `tests/client.test.ts` |
| Throwing or asynchronously rejecting application loggers could escape callback/watch isolation or stop the bounded dispatcher. | Wrapped the client logger as non-interfering telemetry, explicitly continued every dispatcher outcome, and observed Promise/hostile-thenable failures (`ae535de`, `66b8bc6`). | `tests/client.test.ts`, `tests/stress.test.ts` |
...
| Namespace scope, background reconciliation, and deleted-path history were append-only after local unsubscribe or unique-key churn; in-flight reconcile work could repopulate a released scope or apply an old absence set after same-namespace remove/re-add; and delete-only unknown keys bypassed the original compaction path. | Added namespace reference ownership, idle stream/reconcile suspension, tombstone compaction, and a global stale-reconcile fence (`1b90753`); fenced each awaited page by captured scope generation and current ownership (`0a4bd60`); fenced the final absence phase against scope ABA (`b131946`); and pruned unchanged unknown tombstones without inventing callbacks (`a0c7943`). | `tests/stress.test.ts`, `tests/watch.test.ts` |
...
| A rejected initial release registration write lost the already-created bidi stream, while zero-delay jitter could amplify retries into a hot loop. | Cancel the locally owned stream when registration fails (`ae535de`) and impose a deterministic positive reconnect floor (`cbf006e`). | `tests/client-release.test.ts`, `tests/releases/loader.test.ts` |
...
| The descriptor size check ran only after full allocation and UTF-8 decoding silently replaced malformed bytes. | Preflight known sizes, bound every read to `max + 1`, support stat-zero virtual files, and decode UTF-8 fatally (`fc87996`, `222e0f5`, `aca5424`). | `sdk/typescript/tests/configgen.test.ts` |
| Secret-aware cloning detached plaintext but duplicated repeated `Secret` and `ReleaseSecret` aliases despite the shared-reference guarantee. | Record detached secret clones in the graph identity map (`e824434`). | `sdk/typescript/tests/configstore-runtime.test.ts` |

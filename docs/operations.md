...
| `release diff` | `{from: {name, schema_version, version}, to: {name, schema_version, version}, added, removed, changed}` — `added`/`removed` are release entries, `changed` is `{alias, from, to}`. Entry-only: it compares the pinned entries and reads no values. The console's comparison page also carries parameter values and per-version authorship through `GET /api/v1/releases/diff` ([`http-api.md`](http-api.md#release-diff)) |

...
| Reads/writes and credentials | `get/put/list/delete` methods | Bearer auth metadata, operation-specific per-secret request fields, deadlines, exact `bigint` values, and positive protocol request/response coverage for every public metadata/mutation method | `sdk/typescript/tests/client.test.ts`, `sdk/typescript/tests/grpc-integration.test.ts` | complete |
...
| Release exact resolution and digest | `ReleaseLoader` | Deterministic protobuf digest; server-returned ref/version/content/digest verification | `sdk/typescript/tests/releases/digest.test.ts`, `loader.test.ts`, `client-release.test.ts` | complete |
...
| Managed drift/restart policy | `startManagedConfig`, `ManagedConfigManager` | Non-fatal reported startup drift, runtime restore, whole-candidate restart rejection | `sdk/typescript/tests/configstore-manager.test.ts` | complete |

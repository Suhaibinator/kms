...
export function normalizeDescriptor(value: unknown): ConfigDescriptor {
  ...
  const moduleName = normalizeModuleSpecifier(source.module, "$.source.module");
  const typeName = expectNonemptyString(source.type, "$.source.type");
  if (!TYPE_NAME_PATTERN.test(typeName) || TYPESCRIPT_RESERVED_IDENTIFIERS.has(typeName)) {
    throw descriptorError("$.source.type must be a non-reserved TypeScript identifier");
  }
  ...
  switch (raw.kind) {
    ...
    case "fixedArray": {
      ...
      if (length < 1 || length > 1_000_000) {
        throw descriptorError(`${path}.length must be from 1 through 1000000`);
      }
      return freeze({
        kind: "fixedArray",
        element: normalizeType(raw.element, `${path}.element`, depth + 1),
...
export function normalizeModuleSpecifier(value: unknown, path = "module specifier"): string {

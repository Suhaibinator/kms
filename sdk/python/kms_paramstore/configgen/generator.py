...
def _encoding(annotation: Any) -> str:
    ...
    if core is dt.timedelta:
        raise TypeError("configgen: datetime.timedelta is not a portable Go-duration; use Duration")
...
def _schema_for(annotation: Any) -> dict[str, Any]:
    ...
    if core is bool:
        ...
        if any(
            ...
            for item in metadata for name in ("min_length", "max_length")
        ):
            raise TypeError(
                "configgen: decoded byte length constraints are not representable in JSON Schema"
            )
        schema = {"type": "string", "format": "kms-base64"}
    elif core is Duration:

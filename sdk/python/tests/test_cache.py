...
def _client(addr, ttl):
    return Client(addr, cache_ttl=ttl)
...
def test_secret_cache(server):
    ...
    try:
        ...
        store.secrets["/c/s"]["versions"].append((b"two", "application/octet-stream"))
        assert c.get_secret("/c/s").value == b"one"

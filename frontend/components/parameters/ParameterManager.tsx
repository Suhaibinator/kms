...
export default function ParameterManager({
  ...
  const ready = resourceRef !== undefined || queryReady;
  ...
  const mutationRequest = useLatestRequest();
  useEffect(() => {
    void ref;
    return () => {
      viewRequest.abort();
    ...
    return (
      ...
      >
        {loadState === "not-found" ? (
          ...
          <>
            <EmptyState title="Parameter unavailable">
              The server could not load {displayPath(ref)}.
            </EmptyState>
                    ...
                    return (
                      <tr key={v.version} aria-current={isViewed ? "true" : undefined}>
                        ...
                        <td data-label="Created by">
                          {v.created_by || <span className="faint">—</span>}
                        </td>

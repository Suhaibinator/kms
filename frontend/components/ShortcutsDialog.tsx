...
export default function ShortcutsDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      {/* One wrapper so every group's key column resolves against the same
          track set (the groups are subgrids of this one), instead of each
          <dl> sizing `max-content` against only its own chips. */}

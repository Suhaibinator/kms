...
func TestLoopbackDefaultsReleaseCreationCarriesSecretAndStaysInactive(t *testing.T) {
	...
	if _, err := releases.GetActiveRelease(authCtx, &kmsv1.GetActiveReleaseRequest{SchemaVersion: new(schema.GetVersion()),
		Namespace: networkNS(ns.Env, ns.App), Name: "runtime",
	}); status.Code(err) != codes.NotFound {
		t.Fatalf("release creation unexpectedly activated v1: %v", err)
	}
	zero := uint64(0)
	activatedFirst, err := releases.ActivateRelease(authCtx, &kmsv1.ActivateReleaseRequest{SchemaVersion: new(schema.GetVersion()),
	...
	stillActive, err := releases.GetActiveRelease(authCtx, &kmsv1.GetActiveReleaseRequest{SchemaVersion: new(schema.GetVersion()),
		Namespace: networkNS(ns.Env, ns.App), Name: "runtime",
	})
	...
	activatedNext, err := releases.ActivateRelease(authCtx, &kmsv1.ActivateReleaseRequest{SchemaVersion: new(schema.GetVersion()),

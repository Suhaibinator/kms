...
func TestAuditGeneration(t *testing.T) {
	...
	_, _ = h.svc.GetParameter(ctx, nobody, h.ref("/prod/app/param"), 0, "")
	...
	for _, ev := range events {
		...
		blob := ev.EventType + "\x00" + ev.ResourceEnv + "\x00" + ev.ResourceApp + "\x00" +
			ev.ResourceKey + "\x00" + ev.Metadata + "\x00" + ev.ActorIdentity + "\x00" + ev.Decision
...
func TestAuditEventCoverage(t *testing.T) {
	...
	if _, err := h.svc.DisableSecret(ctx, h.admin, ref, 2, false); err != nil {
		t.Fatalf("DisableSecret: %v", err)
	}
	if _, err := h.svc.DisableSecret(ctx, h.admin, ref, 2, true); err != nil {
		t.Fatalf("EnableSecret: %v", err)
	}
	if _, err := h.svc.DestroySecretVersion(ctx, h.admin, ref, 3); err != nil {
		t.Fatalf("DestroySecretVersion: %v", err)
	}

	const pparam = "/prod/app/covered-param"
	mustPutParam(t, h, pparam, "1")
	if _, err := h.svc.DeleteParameter(ctx, h.admin, h.ref(pparam)); err != nil {
...
func TestAuditRowsHaveNoPlaintext(t *testing.T) {
	...
	h.withRawDB(func(db *sql.DB) {
		rows, err := db.QueryContext(ctx,
			"SELECT event_type, metadata_json, resource_env, resource_app, resource_key FROM audit_events")
		if err != nil {
		...
		n := 0
		for rows.Next() {
			var et, md, renv, rapp, rkey string
			if err := rows.Scan(&et, &md, &renv, &rapp, &rkey); err != nil {
				t.Fatalf("scan: %v", err)
			}
			n++
			if strings.Contains(et+md+renv+rapp+rkey, plaintext) {

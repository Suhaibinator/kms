...
func TestSecretPutGetFlow(t *testing.T) {
	...
	r1, err := h.svc.PutSecret(ctx, h.admin, h.stdSecret(path, "v1-secret"))
	if err != nil {
	...
	r2, err := h.svc.PutSecret(ctx, h.admin, h.stdSecret(path, "v2-secret"))
	if err != nil {
	...
	cur2, prevVer, _, err := h.svc.PromoteSecretVersion(ctx, h.admin, h.ref(path), 1)
	if err != nil {
	...
	info, err := h.svc.GetSecretInfo(ctx, h.admin, h.ref(path))
	if err != nil {
	...
	secrets, _, err := h.svc.ListSecrets(ctx, h.admin, nsRef("prod", "payments"), "", storage.ListPage{})
	if err != nil {
		t.Fatalf("ListSecrets: %v", err)
	}
	if len(secrets) != 1 || secrets[0].Ref.String() != path {
...
func TestParameterPutGetFlow(t *testing.T) {
	...
	v1, _, err := h.svc.PutParameter(ctx, h.admin, ref, "100", "integer", "")
	if err != nil {
	...
	got, err := h.svc.GetParameter(ctx, h.admin, ref, 0, "")
	if err != nil || got.Value != "100" {
		t.Fatalf("GetParameter = %q err=%v, want 100", got.Value, err)
	}

	if _, _, err := h.svc.PutParameter(ctx, h.admin, ref, "200", "integer", ""); err != nil {
		t.Fatalf("PutParameter v2: %v", err)
	}
	cur, err := h.svc.GetParameter(ctx, h.admin, ref, 0, domain.LabelCurrent)
	if err != nil || cur.Value != "200" {
		t.Fatalf("current = %q err=%v, want 200", cur.Value, err)
	}
	prev, err := h.svc.GetParameter(ctx, h.admin, ref, 0, domain.LabelPrevious)
	...
	if _, _, err := h.svc.PutParameter(ctx, h.admin, ref, "not-a-number", "integer", ""); !errors.Is(err, domain.ErrInvalidArgument) {
		t.Errorf("bad integer value err = %v, want ErrInvalidArgument", err)
	}

	params, _, err := h.svc.ListParameters(ctx, h.admin, nsRef("prod", "payments"), "", storage.ListPage{})
	if err != nil || len(params) != 1 {
		t.Fatalf("ListParameters = %+v err=%v, want 1", params, err)
	}

	if _, err := h.svc.DeleteParameter(ctx, h.admin, ref); err != nil {
		t.Fatalf("DeleteParameter: %v", err)
	}
	if _, err := h.svc.GetParameter(ctx, h.admin, ref, 0, ""); !errors.Is(err, domain.ErrNotFound) {
...
func TestParameterVersionHistory(t *testing.T) {
	...
	for _, v := range []string{"1", "2", "3"} {
		if _, _, err := h.svc.PutParameter(ctx, h.admin, ref, v, "integer", ""); err != nil {
			t.Fatalf("PutParameter %s: %v", v, err)
		}
	}
	info, err := h.svc.GetParameterInfo(ctx, h.admin, ref)
	if err != nil {
...
func TestTokenAuthentication(t *testing.T) {
	...
	res, err := h.svc.CreateIdentity(ctx, h.admin, core.CreateIdentityInput{
		Name: "billing", Kind: domain.IdentityKindClient,

...
func TestKMSVerifyOverRealKMS(t *testing.T) {
	...
	t.Run("verify-only identity has no read access", func(t *testing.T) {
		...
		if _, err := releases.GetActiveRelease(verifyCtx, &kmsv1.GetActiveReleaseRequest{SchemaVersion: new(uint64(1)), Namespace: networkNS(f.ns.Env, f.ns.App), Name: kmsverifyRelease}); status.Code(err) != codes.PermissionDenied {
			t.Fatalf("GetActiveRelease as verify-only = %v, want PermissionDenied", err)
		}
		if _, err := releases.GetRelease(verifyCtx, &kmsv1.GetReleaseRequest{SchemaVersion: new(uint64(1)), Namespace: networkNS(f.ns.Env, f.ns.App), Name: kmsverifyRelease, Version: driftedVersion}); status.Code(err) != codes.PermissionDenied {

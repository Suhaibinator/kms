...
func (r *bindingRenderer) renderDefaultsArtifactEncoder() {
	r.line("// EncodeDefaultsArtifact encodes a complete parameter-only defaults artifact for profile.")
	r.line("// Secret contract metadata is included, but secret values are structurally excluded.")
	r.line("func EncodeDefaultsArtifact(profile string, root *%s) ([]byte, error) {", r.rootTypeString)
	r.line("\tgroups, err := EncodeParameterGroups(root)")
	r.line("\tif err != nil {")
	r.line("\t\treturn nil, err")
	r.line("\t}")
	r.line("\tparameters := make([]configstore.DefaultsParameter, 0, len(groups))")
	r.line("\tfor _, entry := range generatedContract {")
	r.line("\t\tif entry.Kind != configstore.ContractKindParameter {")
	r.line("\t\t\tcontinue")
	r.line("\t\t}")
	r.line("\t\tvalue, ok := groups[entry.Alias]")
	r.line("\t\tif !ok {")
	r.line("\t\t\treturn nil, fmt.Errorf(%s, entry.Alias)", strconv.Quote("generated config store: missing encoded parameter group %s"))
	r.line("\t\t}")
	r.line("\t\tparameters = append(parameters, configstore.DefaultsParameter{Alias: entry.Alias, ContentType: entry.ContentType, Value: string(value)})")
	r.line("\t}")
	r.line("\treturn configstore.EncodeDefaultsArtifact(configstore.DefaultsArtifact{")
	r.line("\t\tFormat: configstore.DefaultsArtifactFormat,")
	r.line("\t\tProfile: profile,")

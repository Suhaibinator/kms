...
func TestBindUnbindAndRotateDEKPreserveValueCiphertextAndKEK(t *testing.T) {
	kek := mustKEK(t, "kek-mutations")
	aad := BuildAAD("prod", "app", "api", 7)
	original, err := Encrypt(kek, []byte("stable-value"), aad)
	if err != nil {
		t.Fatalf("Encrypt: %v", err)
	}
	valueCiphertext := bytes.Clone(original.Ciphertext)

	bound, err := BindDEK(kek, original.EncryptedDEK, aad, testBindingKeyA)
	if err != nil {
		t.Fatalf("BindDEK: %v", err)
	}
	assertRewrapMetadata(t, bound, kek.ID, domain.WrapModeBindingKey, true)
	assertValueCiphertext(t, original.Ciphertext, valueCiphertext)
	assertDecryptsWithRewrap(t, kek, original, bound, testBindingKeyA, "stable-value")

	if err := TestBindingKeyDEK(kek, bound.EncryptedDEK, bound.BindingKeySalt, aad, testBindingKeyA); err != nil {
		t.Fatalf("TestBindingKeyDEK(correct): %v", err)
	}
	if err := TestBindingKeyDEK(kek, bound.EncryptedDEK, bound.BindingKeySalt, aad, testBindingKeyB); !errors.Is(err, domain.ErrDecryptFailed) {
		t.Fatalf("TestBindingKeyDEK(wrong) = %v, want ErrDecryptFailed", err)
	}

	rotated, err := RotateBindingKeyDEK(kek, bound.EncryptedDEK, bound.BindingKeySalt, aad, testBindingKeyA, testBindingKeyB)
	if err != nil {
		t.Fatalf("RotateBindingKeyDEK: %v", err)
	}
	assertRewrapMetadata(t, rotated, kek.ID, domain.WrapModeBindingKey, true)
	if bytes.Equal(bound.BindingKeySalt, rotated.BindingKeySalt) {
		t.Fatal("rotation reused binding-key salt")
	}
	assertValueCiphertext(t, original.Ciphertext, valueCiphertext)
	assertDecryptsWithRewrap(t, kek, original, rotated, testBindingKeyB, "stable-value")
	if err := TestBindingKeyDEK(kek, rotated.EncryptedDEK, rotated.BindingKeySalt, aad, testBindingKeyA); !errors.Is(err, domain.ErrDecryptFailed) {
		t.Fatalf("old key still opens rotated DEK: %v", err)
	}

	unbound, err := UnbindDEK(kek, rotated.EncryptedDEK, rotated.BindingKeySalt, aad, testBindingKeyB)
	if err != nil {
		t.Fatalf("UnbindDEK: %v", err)
	}
	assertRewrapMetadata(t, unbound, kek.ID, domain.WrapModeStandard, false)
	assertValueCiphertext(t, original.Ciphertext, valueCiphertext)
	assertDecryptsWithRewrap(t, kek, original, unbound, "", "stable-value")
}

func TestBindingMutationRejectsWrongAndShortKeys(t *testing.T) {
	kek := mustKEK(t, "kek-mutation-errors")
	aad := BuildAAD("prod", "app", "api", 1)
	standard, err := Encrypt(kek, []byte("v"), aad)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := BindDEK(kek, standard.EncryptedDEK, aad, "short"); !errors.Is(err, ErrBindingKeyTooShort) {
		t.Fatalf("BindDEK(short) = %v, want ErrBindingKeyTooShort", err)
	}
	bound, err := BindDEK(kek, standard.EncryptedDEK, aad, testBindingKeyA)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := UnbindDEK(kek, bound.EncryptedDEK, bound.BindingKeySalt, aad, testBindingKeyB); !errors.Is(err, domain.ErrDecryptFailed) {
		t.Fatalf("UnbindDEK(wrong) = %v, want ErrDecryptFailed", err)
	}
...
func TestBindingMutationDoesNotModifyInputs(t *testing.T) {
	kek := mustKEK(t, "kek-inputs")
	aad := BuildAAD("prod", "app", "api", 1)
	standard, err := Encrypt(kek, []byte("v"), aad)
	if err != nil {
		t.Fatal(err)
	}
	standardBlob := bytes.Clone(standard.EncryptedDEK)
	bound, err := BindDEK(kek, standard.EncryptedDEK, aad, testBindingKeyA)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(standard.EncryptedDEK, standardBlob) {
		t.Fatal("BindDEK modified source encrypted DEK")
	}
	boundBlob := bytes.Clone(bound.EncryptedDEK)
	salt := bytes.Clone(bound.BindingKeySalt)
	if _, err := RotateBindingKeyDEK(kek, bound.EncryptedDEK, bound.BindingKeySalt, aad, testBindingKeyA, testBindingKeyB); err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(bound.EncryptedDEK, boundBlob) || !bytes.Equal(bound.BindingKeySalt, salt) {
		t.Fatal("RotateBindingKeyDEK modified source wrapping state")
	}
}
...
func assertRewrapMetadata(t *testing.T, result DEKRewrapResult, kekID, mode string, wantSalt bool) {
	t.Helper()
	if result.KEKID != kekID || result.WrapMode != mode {
		t.Fatalf("rewrap metadata = KEK %q mode %q, want KEK %q mode %q", result.KEKID, result.WrapMode, kekID, mode)
	}
	if wantSalt && len(result.BindingKeySalt) != BindingKeySaltSize {
		t.Fatalf("binding salt length = %d, want %d", len(result.BindingKeySalt), BindingKeySaltSize)
	}
	if !wantSalt && result.BindingKeySalt != nil {
		t.Fatal("standard rewrap returned a binding-key salt")
	}
}

func assertDecryptsWithRewrap(t *testing.T, kek *KEK, original EncryptResult, rewrapped DEKRewrapResult, bindingKey, want string) {
	t.Helper()
	in := decryptInput(original, bindingKey)
	in.EncryptedDEK = rewrapped.EncryptedDEK
	in.WrapMode = rewrapped.WrapMode
	in.BindingKeySalt = rewrapped.BindingKeySalt
...
func assertValueCiphertext(t *testing.T, got, want []byte) {
	t.Helper()
	if !bytes.Equal(got, want) {
		t.Fatal("binding mutation changed value ciphertext")
	}
}
